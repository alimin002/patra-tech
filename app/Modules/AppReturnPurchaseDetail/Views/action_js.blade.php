 ...
 **/
 ...
 $(function(){
	...
	function bindReturnPurchaseItem(){
			 //clear grid data to prevent double record
			 $("#tbody_return_purchase").empty();
			 var data_return_purchase_item=JSON.parse($("#data_return_purchase_item").val());
			 for(var i=0; i<= data_return_purchase_item.length-1; i++){
					var app_raw_material_id = data_return_purchase_item[i].app_raw_material_id;
					var raw_material_name 	= data_return_purchase_item[i].raw_material_name;
				  ...
				  var tr="<tr id=tr-"+i+">"+
									...
									"</td>"+						
								"</tr>";
								$("#tbody_return_purchase").append(tr);
			 }
	...
	*/
	function checkItemExists(app_raw_material_id){
		var result=0;
		 var data_return_purchase_item=JSON.parse($("#data_return_purchase_item").val());
			 for(var i=0; i<= data_return_purchase_item.length-1; i++){
				 if( app_raw_material_id == data_return_purchase_item[i].app_raw_material_id){
					 result =1;
				 }
			 }
	...
	function addToReturnPurchase(){
      //prevent duplicare item
		  ...
		  obj_data_return_purchase_item_new.push(new_data);//variabel not assign textarea value
			...
			var obj_data_return_purchase_item		 = JSON.parse($("#data_return_purchase_item").val());
  ...
  function deleteItem(row_id,app_raw_material_id){
	...
	function doUpdateItem(){
		...
		//obj_data_return_purchase_item.splice(selected_element, 1);
		...
		obj_data_return_purchase_item.splice(start_index, number_of_elements_to_remove,new_data);
		
		//clear #data_return_purchase_item to prevent double value in grid
		$("#data_return_purchase_item").val(JSON.stringify(obj_data_return_purchase_item));
		bindReturnPurchaseItem();
		//document.write(JSON.stringify(data_return_purchase_item));
		$("#modal-edit").modal("hide");
	}
